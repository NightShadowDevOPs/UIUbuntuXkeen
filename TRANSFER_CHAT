...
Актуальный релиз для переноса: **v0.6.22**
...
- `v0.6.22` — build/type-check hotfix после падения CI/Actions на `uiBuild`, `UserTrafficStats`, `ProxyProvider` и `HostRuntimeCard`.
...
- `v0.6.22` — build/type-check hotfix без смены архитектуры;
- `v0.6.19` — page-by-page teardown для `TasksPage.vue`;
- `v0.6.22` — пересборка `SubscriptionsPage.vue` под server publish path;
- `v0.6.22` — зачистка `UsersPage.vue` и `PoliciesPage.vue`;
...
- `v0.6.22`: закрыты build/type-check blockers, которые падали в CI/Actions после `v0.6.19`.
