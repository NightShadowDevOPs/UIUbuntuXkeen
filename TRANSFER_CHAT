...
- в `v0.2.5` в workflow добавлены preflight-диагностика и сохранение CI-логов (`ci-logs/`) для install/type-check/build; при падении install/build хвост лога печатается прямо в консоль GitHub Actions, а полный лог загружается отдельным workflow-артефактом.
