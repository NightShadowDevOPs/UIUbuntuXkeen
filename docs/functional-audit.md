...
3. текущая Ubuntu-линия **UIUbuntuXkeen v0.5.0–v0.6.0**.
...
| Провайдеры: SSL/TLS сертификаты | Срок действия, проверка, ошибки, SSL cache | `router-agent` | **MVP восстановлен частично** | UI/runtime workspace уже работает через compatibility bridge и capability layer; дальше нужны scheduler/history и SQLite state |
