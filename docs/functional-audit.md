...
3. текущая Ubuntu-линия **UIUbuntuXkeen v0.5.0–v0.5.3**.
...
| Провайдеры: SSL/TLS сертификаты | Срок действия, проверка, ошибки, SSL cache | `router-agent` | **Частично / требует Ubuntu service** | Плановые SSL checks, SQLite history, refresh SSL cache |
