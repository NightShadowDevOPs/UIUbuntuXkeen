...
Актуальный релиз для переноса: **v0.5.0**
...
- Текущий релиз линии: **v0.5.0**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Текущий релиз `v0.5.0` подготовлен, но ещё не подтверждён пользователем.
...
- в `v0.4.1` Setup, Edit Backend и раздел «Хост / рантайм» дополнительно показывают каноничный путь к логу Mihomo: `/var/log/mihomo/mihomo.log`.
- в `v0.5.0` карточки провайдеров и панель провайдеров показывают более подробную SSL/TLS-диагностику: источник проверки, URL проверки, дату истечения и ошибку probe, если проверка не удалась.
...
- Mihomo Workspace;
- providers / SSL diagnostics / traffic / rules / subscriptions / diagnostics;
- safe config flow и structured editors остаются в плане, но сейчас не являются ближайшим приоритетом.
...
- `v0.4.1` — observability log-path polish: в Setup, Edit Backend и разделе «Хост / рантайм» показан каноничный лог Mihomo `/var/log/mihomo/mihomo.log`.
- `v0.5.0` — провайдеры: SSL / TLS и диагностика: перенесены и адаптированы проверки сертификатов из старой Xkeen-линии, UI показывает источник, URL и ошибку проверки.
