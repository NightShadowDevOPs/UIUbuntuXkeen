...
Актуальный релиз для переноса: **v0.4.0**
...
- Текущий релиз линии: **v0.4.0**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Текущий релиз `v0.4.0` подготовлен, но ещё не подтверждён пользователем.
...
- в `v0.4.0` Setup, Edit Backend и Runtime получили явную карточку data flow: что UI читает напрямую из Mihomo, а что должно идти через отдельный Ubuntu service.
...
- `v0.4.0` — runtime/setup/observability foundation: наглядная data flow карточка в Setup, Edit Backend и Runtime, чтобы было видно, что читаем напрямую из Mihomo, а что ждём от Ubuntu service.
