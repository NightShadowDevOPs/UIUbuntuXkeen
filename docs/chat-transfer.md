...
- Текущий релиз линии: **v0.6.11**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.11** убирает blocking `ssl_probe_batch` из UI Tasks: SSL обновляется через `ssl_cache_refresh` + `mihomo_providers`, а таблица читает `panelSslNotAfter` из agent cache по URL 3x-ui подписок.
