...
Актуальный релиз для переноса: **v0.6.6**
...
- Текущий релиз линии: **v0.6.6**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.6** довёл раздел **«Задачи»** ближе к каноничной operational-форме: список провайдеров сохраняет реальный порядок provider sources, показывает сводку по subscription URL / SSL-ошибкам / скорым истечениям и не скрывает локально сохранённые ссылки подписок, даже если backend-проверки временно недоступны.
