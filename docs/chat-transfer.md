...
Актуальный релиз для переноса: **v0.5.3**
...
- Текущий релиз линии: **v0.5.3**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.5.3** — шаг по разделу **«Хост»**: UI уже умеет отдельно показывать обзор Ubuntu-хоста, системные сервисы и каноничный лог Mihomo через контур `ubuntu-service`.
