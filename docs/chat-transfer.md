...
- Текущий релиз линии: **v0.6.9**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.9** довёл operational-блок провайдеров в разделе **«Задачи»**: список не схлопывается из-за backend/provider-check ошибки, direct SSL/TLS probe собирает effective URL 3x-ui подписок не только из сохранённой карты, но и из текущих proxy providers / provider metadata, а UI закрепляет каноничную терминологию «ссылка подписки».
