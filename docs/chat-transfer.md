...
Актуальный релиз для переноса: **v0.2.10**
...
- в `v0.2.7` шаг `Install dependencies` теперь сам печатает полный `pnpm install` лог и падает в этом же шаге с реальным exit code, так что текст ошибки должен быть виден прямо в том месте, где произошёл сбой.
...
- `v0.2.10` — вторая зачистка TypeScript build annotations после первого зелёного CI bootstrap; исправлены остаточные ошибки шаблонов и типизации без изменения продуктовой логики.
