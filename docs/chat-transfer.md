...
- В `v0.6.14` direct `ssl_probe_batch` снова убран из основного flow Tasks: проверка идёт через router-side `ssl_cache_refresh` + `mihomo_providers`, а URL 3x-ui подписок продолжают храниться в users-db (`providerPanelUrls`) как общий источник panel SSL для всех устройств.
