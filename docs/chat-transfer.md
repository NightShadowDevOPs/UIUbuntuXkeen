...
Актуальный релиз для переноса: **v0.6.18**
...
- `v0.6.18` — rollback после ошибочного runtime-эксперимента: проект возвращён к честной модели **статический UI + выбранный backend через Setup**.
