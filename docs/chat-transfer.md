...
- Текущий релиз линии: **v0.6.8**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.8** подключил реальный SSL/TLS-опрос провайдеров в разделе **«Задачи»**: кнопки обновления теперь запускают direct probe по сохранённым URL 3x-ui подписок через compatibility bridge, если он доступен, а таблица показывает реальные даты сертификатов и TLS-ошибки именно по subscription URL.
