Prepared release: v0.6.31. The GitHub Actions workflow file is replaced with a simpler log-first variant so CI finally prints the first real install/build error instead of empty `Error:` blocks.
...
Актуальный релиз для переноса: **v0.6.31**
...
Что поймано
- До `v0.6.31` даже после фиксов bootstrap и YAML workflow CI продолжал показывать пустые `Error:` в шагах `Install dependencies` и `Build UI`.
- Значит следующая полезная информация должна прийти уже из inline логов `install-deps.log` и `build-ui.log`.

Следующий шаг

- Прогнать workflow из `v0.6.31` и взять первую реальную ошибку install/build, если CI ещё не зелёный.
