...
Актуальный релиз для переноса: **v0.6.5**
...
- Текущий релиз линии: **v0.6.5**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.5** добил практический хвост по разделу **«Задачи»**: список провайдеров теперь собирается не только из provider-health backend, но и из текущих provider sources UI плюс сохранённых URL подписок/иконок/SSL override-настроек. Дополнительно в `docs/project-spec.md` зафиксирована каноничная структура формы «Задачи» и назначение её кнопок/разделов.
