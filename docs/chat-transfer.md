...
Актуальный релиз для переноса: **v0.6.3**
...
- Текущий релиз линии: **v0.6.3**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.3** — стабилизационный шаг по install/build: добавлен `.npmrc` для обхода падения GitHub Actions на `pnpm install`, версия `prettier-plugin-tailwindcss` возвращена к `^0.6.14`, а следующий функциональный шаг по SSL в **«Задачах»** сознательно отложен до зелёной сборки.
...
- `v0.6.3` — build/install stabilization before next Tasks SSL step
