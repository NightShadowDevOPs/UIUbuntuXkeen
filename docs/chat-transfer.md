...
Актуальный релиз для переноса: **v0.6.2**
...
- Текущий релиз линии: **v0.6.2**
- Последний подтверждённо рабочий релиз на сервере: **v0.2.10**
- Релиз **v0.6.2** — корректирующий шаг по SSL в разделе **«Задачи»**: проверка и отображение сертификатов снова идут по **provider/subscription source URL**, а не по panel URL или через список прокси-провайдеров. Правильный текущий контур такой: список провайдеров остаётся чистым, а SSL-проверки и связанные действия живут в разделе **«Задачи»** и завязаны на существующий provider-health flow.
...
- `v0.6.0` — ошибочный шаг: SSL workspace был добавлен не в тот раздел и затем откатан
- `v0.6.3` — Provider scheduler, history и GEO groundwork
- `v0.6.4` — QoS / shaping foundation
