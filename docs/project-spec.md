...
Актуальная версия документа: **v0.5.4**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.5.4**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**, а в UI появился не только capability-aware foundation для провайдеров, но и новый host-runtime экран под `ubuntu-service`: обзор хоста, сервисы и логи. При этом полноценный operational-перенос всё ещё требует нового **Ubuntu service** с живыми endpoint-ами и scheduler/storage контуром.
