...
Документ соответствует релизу **v0.6.0**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. В UI появился не только capability-aware foundation для провайдеров и host-runtime экран под `ubuntu-service`, но и первый рабочий MVP по SSL/TLS проверке прокси-провайдеров. Полноценный operational-перенос всё ещё требует нового **Ubuntu service** с живыми endpoint-ами, scheduler/history и storage контуром.
