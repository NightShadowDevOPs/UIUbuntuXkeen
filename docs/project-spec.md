...
Актуальная версия документа: **v0.3.2**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.3.2**. На этом этапе уже зафиксированы release-flow новой линии, foundation backend contract, hybrid backend setup flow и runtime/setup/observability preview: Setup и Edit Backend показывают базовый URL, probe/runtime endpoint-ы и каноничные Ubuntu paths для режима `ubuntu-service`, а runtime-раздел начинает смещаться от router-era смысла к host-oriented модели.
...
На релизах `v0.3.0–v0.3.2` в проекте уже зафиксирован foundation backend contract: режимы `compatibility-bridge` и `ubuntu-service`, каноничные Ubuntu paths, стартовый набор endpoint groups, явный выбор backend mode и UI-preview backend contract в setup/edit flow. Это ещё не финальная реализация Ubuntu service, но уже каноничная точка для дальнейшего развития API, hybrid data model, capability-driven UI и observability-модели.
