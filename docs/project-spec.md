...
Актуальная версия документа: **v0.4.0**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.4.0**. На этом этапе уже зафиксированы release-flow новой линии, foundation backend contract, hybrid backend setup flow и runtime/setup/observability foundation: Setup, Edit Backend и Runtime теперь показывают не только backend contract preview, но и явную data flow модель — что UI читает напрямую из Mihomo, а что должно обслуживаться отдельным Ubuntu service.
...
На релизах `v0.3.0–v0.4.0` в проекте уже зафиксирован foundation backend contract: режимы `compatibility-bridge` и `ubuntu-service`, каноничные Ubuntu paths, стартовый набор endpoint groups, явный выбор backend mode, UI-preview backend contract в setup/edit flow и наглядная data flow модель в runtime/setup экранах. Это ещё не финальная реализация Ubuntu service, но уже каноничная точка для дальнейшего развития API, hybrid data model, capability-driven UI и observability-модели.
