...
Документ соответствует релизу **v0.6.2**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. Важная коррекция текущего состояния: в `v0.6.2` SSL-контур в разделе **«Задачи»** переведён на **provider/subscription source URL**. Правильный контур проекта такой: capability-aware foundation для провайдеров и host-runtime экран уже есть, SSL живёт в **«Задачах»**, а дальше должен развиваться через новый **Ubuntu service** с живыми endpoint-ами, scheduler/history и storage контуром.
