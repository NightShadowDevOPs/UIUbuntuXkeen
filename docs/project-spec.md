...
Актуальная версия документа: **v0.2.1**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.2.1**. На этом этапе зафиксированы не только целевая архитектура Ubuntu-линии, но и release-flow проекта: GitHub Actions должны собирать UI, публиковать versioned release и обновлять rolling-архив `dist.zip`, который используется встроенным механизмом обновления UI. После hotfix v0.2.1 шаг `pnpm type-check` переведён в informational-режим, чтобы типизационные хвосты исходной линии не блокировали публикацию рабочего артефакта.
...
На релизах `v0.3.0–v0.3.1` в проекте уже зафиксирован foundation backend contract: режимы `compatibility-bridge` и `ubuntu-service`, каноничные Ubuntu paths, стартовый набор endpoint groups и явный выбор backend mode в setup/edit flow. Это ещё не финальная реализация Ubuntu service, но уже каноничная точка для дальнейшего развития API, hybrid data model и setup flow.
