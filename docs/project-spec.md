...
Актуальная версия документа: **v0.6.7**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.6.7**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. Важная коррекция текущего состояния: раздел **«Задачи»** должен повторять практическую operational-логику старой линии, а не изобретать новую структуру. Для провайдеров задаются URL их **3x-ui подписок**, именно по ним в Tasks выполняется SSL/TLS-опрос сертификатов, и в релизе **v0.6.7** этот опрос уже реально подключён к кнопкам refresh и к operational-таблице Tasks. Сама страница должна показывать список провайдеров, быстрые действия, live logs, GEO/rule-providers, диагностику, upstream updates, users DB и историю операций в одном каноничном экране.
