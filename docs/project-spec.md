> Примечание v0.6.49: пользователь явно подтвердил старт backend-этапа. В проект снова добавлен отдельный каталог `backend/`, но теперь это согласованный standalone Ubuntu service, который запускается отдельно через systemd и подключается к UI через `Setup`.
...
Документ соответствует релизу **v0.6.49**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**.
...
- в `v0.6.49` отдельный Ubuntu backend уже добавлен в репозиторий как согласованный standalone service; его ещё нужно подтвердить на живом хосте.
