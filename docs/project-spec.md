...
Актуальная версия документа: **v0.6.9**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.6.9**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. Важная коррекция текущего состояния: раздел **«Задачи»** должен повторять практическую operational-логику старой линии, а не изобретать новую структуру. Для провайдеров задаются URL их **3x-ui подписок**, именно по ним в Tasks выполняется SSL/TLS-опрос сертификатов, и в релизах **v0.6.8–v0.6.9** этот опрос уже подключён к кнопкам refresh и к operational-таблице Tasks, а список провайдеров не должен исчезать целиком даже при backend/provider-check ошибке. Сама страница должна показывать список провайдеров, быстрые действия, live logs, GEO/rule-providers, диагностику, upstream updates, users DB и историю операций в одном каноничном экране.
