...
Актуальная версия документа: **v0.6.14**  
Дата актуализации: **2026-03-30**

## 0. Статус документа

Документ соответствует релизу **v0.6.14**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. Важная коррекция текущего состояния: раздел **«Задачи»** должен повторять практическую operational-логику старой линии, а не изобретать новую структуру. Для провайдеров задаются URL их **3x-ui подписок**, именно по ним должен обновляться router-side SSL/TLS cache. В релизе **v0.6.14** UI снова не дёргает blocking `ssl_probe_batch` напрямую и использует штатный flow `ssl_cache_refresh` + `mihomo_providers`, а таблица провайдеров читает `panelSslNotAfter` из agent cache. Сама страница должна показывать список провайдеров, быстрые действия, live logs, GEO/rule-providers, диагностику, upstream updates, users DB и историю операций в одном каноничном экране.
