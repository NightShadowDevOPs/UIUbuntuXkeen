> Примечание v0.6.50: пользователь явно подтвердил старт backend-этапа. В проект снова добавлен отдельный каталог `backend/`, но теперь это согласованный standalone Ubuntu service, который запускается отдельно через systemd и подключается к UI через `Setup`.
...
Документ соответствует релизу **v0.6.50**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**.
...
- в `v0.6.50` отдельный Ubuntu backend уже добавлен в репозиторий как согласованный standalone service; его ещё нужно подтвердить на живом хосте.
...
После первого живого подключения standalone backend подтверждено архитектурное правило: `Setup` хранит для Ubuntu backend `secondaryPath=/api`, но фронт не должен дублировать этот путь при вызовах server-side API. Этот hotfix закрепляет контракт `host:port + secondaryPath + endpoint` без повторного `/api`.
