...
На релизе `v0.3.0` в проекте уже зафиксирован foundation backend contract: режимы `compatibility-bridge` и `ubuntu-service`, каноничные Ubuntu paths и стартовый набор endpoint groups. Это ещё не финальная реализация Ubuntu service, но уже каноничная точка для дальнейшего развития API и setup flow.
