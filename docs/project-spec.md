...
Документ соответствует релизу **v0.6.3**. На этом этапе проект уже имеет рабочий release-flow и подтверждённый server update path на релизе **v0.2.10**. Важная коррекция текущего состояния: после функционального шага `v0.6.2`, где SSL-контур в разделе **«Задачи»** был переведён на **provider/subscription source URL**, релиз `v0.6.3` сознательно не расширяет UI-функционал, а стабилизирует install/build и package-manager bootstrap. Правильный дальнейший контур проекта такой: capability-aware foundation для провайдеров и host-runtime экран уже есть, SSL живёт в **«Задачах»**, а дальше должен развиваться через новый **Ubuntu service** с живыми endpoint-ами, scheduler/history и storage контуром.
