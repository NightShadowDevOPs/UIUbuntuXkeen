...
- короткоживущие IP-сертификаты обновляются с жизненным циклом 6 дней, поэтому блок сертификатов должен быть реально рабочим и не ждать только ручного запуска;
- приоритетно довести автопросмотр/опрос сертификатов 3x-ui hosts, так как при просрочке сертификата подписка/подключение может перестать работать.
