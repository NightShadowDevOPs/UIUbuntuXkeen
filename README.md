...
- Релиз `v0.6.37` убирает реальную причину падения CI: из `src/` удалены заехавшие transpiled `.js`/`.d.ts`, которые перекрывали `.ts`/`.tsx`; дополнительно Vite теперь предпочитает TypeScript-резолвинг, автоматическая проверка SSL-сертификатов прокси-провайдеров не тронута.
...
- прогнать GitHub Actions на `v0.6.37`;
...
As of `v0.6.37`, provider certificate checks are **not** treated as a finished frontend-only feature. The UI keeps the provider subscription URL editor, while the actual TLS/SSL polling is reserved for a dedicated Ubuntu service running on the project host.
