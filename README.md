...
- Релиз `v0.6.34` обновляет GitHub Actions до `actions/checkout@v5` / `actions/setup-node@v6`, сохраняет полные install/type-check/build логи в summary и artifact и не трогает автоматическую проверку SSL-сертификатов прокси-провайдеров.
...
- прогнать GitHub Actions на `v0.6.34`;
...
As of `v0.6.34`, provider certificate checks are **not** treated as a finished frontend-only feature. The UI keeps the provider subscription URL editor, while the actual TLS/SSL polling is reserved for a dedicated Ubuntu service running on the project host.
