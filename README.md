...
- Релиз `v0.6.30` упрощает GitHub Actions workflow: install/build шаги больше не прячут первичную ошибку, а CI install временно использует `--no-frozen-lockfile`, чтобы пройти мимо drift между `package.json` и `pnpm-lock.yaml`.
