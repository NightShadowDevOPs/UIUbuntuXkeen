...
export const setPendingPageFocus = (route, kind, value) => {
    const v = String(value || '').trim();
    write({ ts: safeNow(), route: String(route || '').trim(), kind: String(kind || '').trim(), value: v });
};
export const getPendingPageFocusForRoute = (route) => {
    const pf = read();
    if (!pf)
        return null;
    if (safeNow() - pf.ts > TTL_MS) {
    ...
    if (String(pf.route || '').trim() !== String(route || '').trim())
        return null;
    return pf;
};
// Visual highlight helper.
export const flashNavHighlight = (el, durationMs = 2600) => {
    if (!el)
        return;
    try {
