import { BACKEND_KINDS, UBUNTU_BACKEND_ENDPOINTS, UBUNTU_BACKEND_FOUNDATION, } from '@/config/backendContract';
const LOCAL_HOSTS = new Set(['127.0.0.1', 'localhost', '::1', '0.0.0.0']);
export const normalizeSecondaryPath = (value) => {
...
export const detectBackendKind = (backend) => {
...
export const normalizeBackendInput = (backend) => {
...
export const isUbuntuServiceBackend = (backend) => {
    return detectBackendKind(backend) === BACKEND_KINDS.UBUNTU_SERVICE;
};
export const applyRecommendedSecondaryPath = (backend) => {
    return {
        ...backend,
        secondaryPath: getRecommendedSecondaryPath(backend.kind),
    };
};
export const mergeBackendCapabilities = (current, next) => {
    if (!current && !next)
        return undefined;
    return {
        ...(current || {}),
