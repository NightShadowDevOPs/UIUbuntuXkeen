...
let toastRef = null;
export const initNotification = (toast) => {
    toastRef = toast;
};
const setTimer = (alert, timeout, alertKey) => {
    let timer = -1;
    if (timeout !== 0) {
        timer = setTimeout(() => {
            if (alertKey) {
...
const closeAlert = (alert, alertKey) => {
    if (alertKey) {
...
const setAlert = (alert, content, params, type, alertKey) => {
...
export const showNotification = ({ content, params = {}, key, type = 'alert-warning', timeout = 3000, }) => {
    const alertKey = key;
