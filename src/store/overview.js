...
export const lastMemoryTick = ref(0);
export const memoryHistory = ref([...initValue]);
export const connectionsHistory = ref([...initValue]);
export const downloadSpeed = ref(0);
export const lastTrafficTick = ref(0);
export const uploadSpeed = ref(0);
...
export const initSatistic = () => {
    ...
    const unwatchMemory = watch(() => memoryWsData.value, (data) => {
        if (!data)
            return;
    ...
    const unwatchTraffic = watch(() => trafficWsData.value, (data) => {
        if (!data)
            return;
        const timestamp = Date.now().valueOf();
