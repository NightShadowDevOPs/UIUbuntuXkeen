...
export const autoSortProxyProvidersByHealth = useStorage('config/auto-sort-proxy-providers-by-health', true);
export const proxyProvidersSortMode = useStorage('config/proxy-providers-sort-mode', 'health');
export const showOnlyActiveProxyProviders = useStorage('config/show-only-active-proxy-providers', false);
export const showOnlyTrafficProxyProviders = useStorage('config/show-only-traffic-proxy-providers', false);
export const providerHealthFilter = useStorage('config/provider-health-filter', '');
const _proxyProvidersProtoFilter = useStorage('config/proxy-providers-proto-filter', 'all');
...
const hasUbuntuProviderCapability = (...keys) => {
...
export const agentProvidersError = ref(null);
export const agentProvidersAt = ref(0);
export const agentProviders = ref([]);
...
export const agentProvidersSslCacheAgeSec = ref(-1);
export const agentProvidersSslCacheNextRefreshAtMs = ref(0);
export const agentProvidersNextCheckAtMs = ref(0);
export const agentProvidersJobStatus = ref('');
export const agentProvidersJobId = ref('');
export const panelSslNotAfterByName = ref({});
export const panelSslErrorByName = ref({});
export const panelSslUrlByName = ref({});
export const panelSslCheckedAt = ref(0);
export const panelSslProbeError = ref(null);
export const panelSslProbeLoading = ref(false);
export const agentProviderByName = computed(() => {
    const map = {};
...
const resetProviderRuntimeState = (error = null) => {
...
const normalizeSavedProviderSubscriptionUrl = (raw) => {
...
const collectSavedProviderSubscriptionTargets = () => {
    ...
    for (const provider of (proxyProviederList.value || [])) {
        ...
        if (!name || name === 'default' || provider?.vehicleType === 'Compatible')
            continue;
        pushTarget(name);
    }
...
export const fetchAgentProviders = async (force = false) => {
    ...
    if (agentProvidersLoading.value)
        return;
    ...
    try {
        const checksPromise = hasUbuntuProviderCapability('providerChecks', 'providers')
            ...
            : Promise.resolve({ ok: false, providers: [], error: '' });
        ...
        const checks = checksRes.status === 'fulfilled' ? checksRes.value : { ok: false, providers: [], error: checksRes.reason?.message || 'failed' };
        const cache = cacheRes.status === 'fulfilled' ? cacheRes.value : { ok: false, error: cacheRes.reason?.message || '' };
...
export const refreshSavedProviderSubscriptionSsl = async () => {
    ...
    return { ok: false, error, checkedAtSec: Math.floor(Date.now() / 1000), items: [] };
};
...
export const refreshAgentProviderSslCache = async () => {
    ...
    let res = { ok: false, error: 'capability-missing' };
    try {
...
watch([activeBackend, activeBackendCapabilities], () => {
    fetchAgentProviders(false);
}, { immediate: true, deep: true });
watch(proxyProviderSubscriptionUrlMap, () => {
