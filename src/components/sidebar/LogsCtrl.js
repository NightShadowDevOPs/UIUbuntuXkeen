...
import { ArrowDownTrayIcon, PauseIcon, PlayIcon, WrenchScrewdriverIcon, XMarkIcon, } from '@heroicons/vue/24/outline';
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const insertLogSearchHistory = debounce((log) => {
            if (!log) {
                return;
            }
        ...
        const logFilterOptions = computed(() => {
            ...
            return {
                levels: levels.sort((a, b) => {
                    const aIdx = logLevels.value.indexOf(a);
                    const bIdx = logLevels.value.indexOf(b);
                    return aIdx - bIdx;
                }),
                types: types.sort(),
            };
        ...
        return () => {
            const levelSelect = (<select class={['join-item select select-sm']} v-model={logLevel.value} onChange={initLogs}>
          {logLevels.value.map((opt) => (<option key={opt} value={opt}>
              {opt}
            </option>))}
        </select>);
            const searchInput = (<TextInput v-model={logFilter.value} beforeClose={true} class="flex-1" placeholder={`${t('search')} | Regex`} clearable={true} menus={logSearchHistory.value} menusDeleteable={true} onUpdate:menus={(val) => (logSearchHistory.value = val)}/>);
            const logTypeSelect = (<select class={[
          ...
          <optgroup label={t('logLevel')}>
            {logFilterOptions.value.levels.map((opt) => (<option key={opt} value={opt}>
                {opt}
              </option>))}
          </optgroup>
          <optgroup label={t('logType')}>
            {logFilterOptions.value.types.map((opt) => (<option key={opt} value={opt}>
                {opt}
              </option>))}
          </optgroup>
        </select>);
          ...
          <DialogWrapper v-model={settingsModel.value}>
            <div class="flex flex-col gap-4 p-2 text-sm">
              <div class="flex items-center gap-2">
                {t('logRetentionLimit')}
                <input class="input input-sm w-20" type="number" max="9999" v-model={logRetentionLimit.value}/>
              </div>
            </div>
          </DialogWrapper>
        </>);
          ...
          <button class="btn btn-circle btn-sm" onClick={() => (logs.value = [])}>
            ...
            return (<div class="flex items-center justify-between gap-2 p-2">
          <div class="flex items-center gap-2">
            {levelSelect}
            <div class="join w-96">
              {logTypeSelect}
