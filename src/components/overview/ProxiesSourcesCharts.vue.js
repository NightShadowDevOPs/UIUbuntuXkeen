...
import { blurIntensity, dashboardTransparent, font, proxiesRelationshipColorMode, proxiesRelationshipPaused, proxiesRelationshipRefreshNonce, proxiesRelationshipRefreshSec, proxiesRelationshipSourceMode, proxiesRelationshipTopN, proxiesRelationshipTopNChain, proxiesRelationshipWeightMode, theme, } from '@/store/settings';
...
const sourceFromConnection = (c) => {
    ...
    if (mode === 'rulePayload')
        return normalize(c.rulePayload);
    if (mode === 'host')
        return normalize(c.metadata.host);
    if (mode === 'destinationIP')
        return normalize(c.metadata.destinationIP);
...
const sankeyData = computed(() => {
    ...
    const topSources = new Set(Array.from(sourceTotals.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, topSourcesN)
        .map(([k]) => k));
...
const render = (force = false) => {
    if (!myChart)
        return;
    myChart.setOption(options.value, { notMerge: force, lazyUpdate: true });
