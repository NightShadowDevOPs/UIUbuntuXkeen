...
import { hideUnavailableProxies, proxyCountMode, proxySortType, useSmartGroupSort, } from '@/store/settings';
import { smartWeightsMap } from '@/store/smart';
import { computed } from 'vue';
export function useRenderProxies(proxies, proxyGroup) {
...
const getRenderProxies = (proxies, groupName) => {
    ...
    if (useSmartGroupSort.value && smartWeightsMap.value[groupName]) {
        ...
        return proxies.sort((prev, next) => {
            return (smartGroupSort.indexOf(smartWeightsMap.value[groupName][prev]) -
    ...
    const getLatencyForSort = (name) => {
        const latency = latencyMap.get(name);
        return latency === 0 ? Infinity : latency;
    };
    const sortFuncMap = {
        [PROXY_SORT_TYPE.NAME_ASC]: (prev, next) => prev.localeCompare(next),
        [PROXY_SORT_TYPE.NAME_DESC]: (prev, next) => next.localeCompare(prev),
        [PROXY_SORT_TYPE.LATENCY_ASC]: (prev, next) => getLatencyForSort(prev) - getLatencyForSort(next),
        [PROXY_SORT_TYPE.LATENCY_DESC]: (prev, next) => getLatencyForSort(next) - getLatencyForSort(prev),
    };
