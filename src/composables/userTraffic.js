...
const bucketKey = (ts) => dayjs(ts).format('YYYY-MM-DDTHH');
const looksLikeIP = (s) => {
    const v = (s || '').trim();
    if (!v)
        return false;
...
export const getUserHourBucket = (user, ts = Date.now()) => {
...
const add = (user, dl, ul, ts = Date.now()) => {
    if (!user)
        return;
...
export const initUserTrafficRecorder = () => {
    ...
    watch(activeConnections, (list) => {
        ...
        for (const c of list) {
            ...
            if (d === 0 && u === 0) {
                const sd = Number(c?.downloadSpeed ?? 0) || 0;
                const su = Number(c?.uploadSpeed ?? 0) || 0;
                if (sd > 0 || su > 0) {
                    d = sd;
                    u = su;
                }
...
export const getTrafficRange = (startTs, endTs) => {
...
const groupKeyForHour = (hourKey, groupBy) => {
    const t = dayjs(hourKey, 'YYYY-MM-DDTHH');
    if (groupBy === 'day')
        return t.startOf('day').format('YYYY-MM-DD');
    if (groupBy === 'month')
        return t.startOf('month').format('YYYY-MM');
    // week
    return isoWeekStart(t).format('YYYY-MM-DD');
};
export const getTrafficGrouped = (startTs, endTs, groupBy) => {
...
export const formatTraffic = (bytes) => prettyBytesHelper(bytes);
