...
export const useSwipeRouter = () => {
    ...
    const swipeList = computed(() => {
        return flatten(renderRoutes.value.map((r) => {
    ...
    const getPrevRouteName = () => {
        ...
        return swipeList.value[(getNextIndexInSwipeList() - 1 + swipeList.value.length) % swipeList.value.length]?.[1]?.();
    };
    const isInputActive = () => {
