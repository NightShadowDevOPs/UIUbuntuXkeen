...
- SSL в разделе **«Задачи»** снова работает через router-side flow `ssl_cache_refresh` + `mihomo_providers`, без blocking direct `ssl_probe_batch` из браузера;
- URL 3x-ui подписок остаются общими через users-db (`providerPanelUrls`) и используются agent'ом как единый серверный источник panel SSL для всех устройств;
- router-agent получил более надёжный разбор HTTPS/WSS URL для TLS-проверок: корректнее обрабатываются query-string, fragment, userinfo и нестандартные варианты `host:port`.
